...
{
    public class UsuarioService : IUsuarioService //clase que gestiona las operaciones relacionadas con los usuarios
    {
        ...
        public async Task<List<Usuario>> Lista() //devuelve la lista de usuarios de forma asincrona
        {
            IQueryable<Usuario> query = await _repositorio.Consultar();
            return query.Include(r => r.IdRolNavigation).ToList();
        }

        public async Task<Usuario> Crear(Usuario entidad, Stream Foto = null, string NombreFoto = "", string UrlPlantillaCorreo = "") //crea un nuevo usuario
        {
                ...
                if (UrlPlantillaCorreo != "")
                {
                    ...
                    if (htmlCorreo != "")
                        await _correoService.EnviarCorreo(
                            ...
                            "Cuenta Creada - Sistema de Ventas",
                            htmlCorreo
                            );
                }
        ...
        public async Task<Usuario> Editar(Usuario entidad, Stream Foto = null, string NombreFoto = "") //edita un usuario existente
        {
        ...
        public async Task<bool> Eliminar(int IdUsuario) //elimina un usuario existente
        {
            try
            {
        ...
        public async Task<Usuario> ObtenerPorCredenciales(string correo, string clave) //obtiene un usuario por sus credenciales
        {
        ...
        public async Task<Usuario> ObtenerPorId(int IdUsuario) //obtiene un usuario por su id
        {
        ...
        public async Task<bool> GuardarPerfil(Usuario entidad) //guarda el perfil de un usuario existente
        {
            try
            {
        ...
        public async Task<bool> CambiarClave(int idUsuario, string claveActual, string ClaveNueva) //cambia la clave de un usuario existente
        {
            try
            {
                Usuario usuario_encontrado = await _repositorio.Obtener(u => u.IdUsuario == idUsuario);
        ...
        public async Task<bool> RestablecerClave(string Correo, string UrlPlantillaCorreo) //restablece la clave de un usuario existente
        {
            try
            {
                ...
                UrlPlantillaCorreo = UrlPlantillaCorreo
                    .Replace("[correo]", usuario_encontrado.Correo)
                ...
                if (htmlCorreo != "")
                {
                    bool enviado = await _correoService.EnviarCorreo(
                        Correo,
