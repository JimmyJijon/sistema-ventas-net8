...
$("#cboBuscarProducto").on("select2:select", function (e) {
    ...
    const data = e.params.data; // Trae todos los elementos que encontró la peticion ajax dentro de #cboBuscarProducto

    let producto_encontrado = ProductosParaVenta.filter(p => p.idProducto == data.id) // valida que el producto no pueda ser seleccionado mas de una vez 
...
function mostrarProducto_Precios() { // funcion que pinta la data en la tabla una vez que se selecciono un producto en el select y se le ingreso una cantidad
    // tambien pinta los valores en la tabla de total, subtotal y valor de impuesto
    let total = 0;
    let igv = 0;
    let subtotal = 0;
...
$(document).on("click", "button.btn-eliminar", function () { // Logica para boton de eliminar en la tabla de productos seleccionados
    const _idproducto = $(this).data("idProducto")

    ProductosParaVenta = ProductosParaVenta.filter(p => p.idProducto != _idproducto); // excluye el id seleccionado

    mostrarProducto_Precios(); // vuelve a dibujar la data
